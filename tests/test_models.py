...
def test_compact_recovery_title_supplies_missing_company():
    o = Opportunity.from_dict({
        "stable_id": "amazon:10496769", "issue": 36, "stage": "qualified",
        "title": "Amazon — Data Engineer Intern 2027", "kind": "internship",
    })
...
def test_compact_recovery_accepts_full_historical_shape():
    o = Opportunity.from_dict({
        "stable_id": "legacy:42", "issue": 42, "status": "open", "stage": "qualified",
        "title": "Example — ML Intern", "kind": "internship", "fit": 8.0,
        "confidence": 0.9, "priority": 8.5, "conversion": "explicit",
        "verified_at": "2026-09-22T00:00:00+00:00", "verified_open": True,
...
def test_monthly_compensation_recovery_is_backward_compatible():
    o = Opportunity.from_dict({
        "id": "x", "company": "Example", "title": "Intern", "kind": "internship",
        "source": "test", "canonical_url": "https://example.com/jobs/x",
...
def test_compact_daily_salary_and_source_url_recovery():
    o = Opportunity.from_dict({
        "stable_id": "ashby:example:42", "issue": 42, "stage": "qualified",
        "title": "Example — Software Engineer New Grad", "kind": "full_time",
        "source": "https://jobs.ashbyhq.com/example/42", "verified_open": "2026-09-23",
        "compensation": {
            "currency": "USD", "min": 123000, "max": 158000,
            "min_inr_lpa": 117.58, "fx_date": "2026-09-22",
            "fx_source": "ECB", "usd_inr": 95.595,
        },
    })
    assert o.source == "ashby"
    assert o.canonical_url == "https://jobs.ashbyhq.com/example/42"
    assert o.compensation.min_annual == 123000
    assert o.compensation.max_annual == 158000
    assert o.compensation.min_lpa_inr == pytest.approx(117.58)
    assert o.compensation.fx_rate == pytest.approx(95.595)
    assert o.compensation.fx_rate_date == "2026-09-22"
